    ...
    public function login(Request $request)
    {
        ...
        if ($user && $user->status === 'verify') {
            if (password_verify($request->password, $user->password)) {
                ...
                if ($user->role === 'admin') {
                    return redirect()->route('npp.pengajuan')->with('alert', [
                        'type' => 'success',
                        'title' => 'Login',
                        'text' => 'Login berhasil',
                    ]);
            } elseif ($user->role === 'sarana') {
                return redirect()->route('rd.pengajuan')->with('alert', [
                    'type' => 'success',
                    'title' => 'Login',
                    'text' => 'Login berhasil',
                ]);
            ...
            return redirect()->route('auth')->with('alert', [
                'type' => 'error',
                'title' => 'Login Gagal',
                'text' => 'Akun Anda belum diverifikasi oleh Admin / Akun Anda Tidak Ditemukan '
            ]);
        }
    }

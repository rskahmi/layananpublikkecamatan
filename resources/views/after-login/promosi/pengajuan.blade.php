...
@section('content')
                                ...
                                <td>
                                    <div class="aksi">
                                        @if (isAllAdmin())
                                            @if (isAVPAdm())
                                                <a href="##edit" id="edit"
                                                    onclick="modalEditPromosi(
                                                        '{{ route('promosi.update', ['id' => $item->id]) }}',
                                            ...
                                            <a href="{{ route('promosi.detail', ['id' => $item->id]) }}">
                                                <x-svg.icon.info />
                                            </a>
                                            {{-- @if (isAVPAdm())
                                                <x-layout.delete action="{{ route('promosi.destroy', ['id' => $item->id]) }}" />
                                            @endif --}}
                                        @endif
                                    </div>
                                </td>
    ...
    @if (isAllAdmin())
        <!-- Modal -->
        <x-modals.admin action="{{route('promosi.store')}}">
            @slot('slotHeader')
        ...
        <x-modals.admin action="{{route('promosi')}}" id="editModal" isUpdate=true>
            @slot('slotHeader')

    ...
    <div class="row detail-pengajuan">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-6 mt-2 mt-sm-2 mt-md-3">
                            <x-text.PopUpMenu title="Tanggal Diajukan" subtitle="{{ format_dfy($promosi->tanggal) }}" />
                        </div>
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-6 mt-2 mt-sm-2 mt-md-3 popup-text">
                            <h6>Memo TEO </h6>
                            <a target="_blank" href="{{ asset('storage/promosi/' . $promosi->memoteo) }}">
                        ...
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-6 mt-2 mt-sm-2 mt-md-3 popup-text">
                            ...
                            <span
                                ...
                                {{ isStatusDiterima($promosi->status) ? 'bg-success' : (isStatusProses($promosi->status) ? 'bg-warning' : (isStatusDitolak($promosi->status) ? 'bg-danger' : 'bg-warning-subtle text-dark')) }}">{{ $promosi->status }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div
            ...
            <div class="container ms-0 ms-sm-0 ms-md-0 ms-lg-0 ms-xl-5">
                ...
                @if (isset($riwayat_promosi))
                    @if (count($riwayat_promosi) > 0)
                        <div
                            class="row {{ (isStatusDiterima($promosi->status) || isStatusDitolak($promosi->status)) ? 'mt-5' : '' }}">
                            <div class="col-12">
                                <h6 class="riwayat-header">Riwayat</h6>
                            </div>
                            <div class="col-12 mt-3">
                                <ul class="timeline">
                                    @foreach ($riwayat_promosi as $item)
        ...
        <x-modals.admin id="modalVerifikasiPengajuan" action="{{ route('promosi.verifikasi', ['id' => $promosi->id]) }}"
            class="modal-xl">
            @slot('slotHeader')
                <h5 class="modal-title" id="exampleModalLabel">
                    @if ($promosi !== null && isStatusDiajukan($promosi->status))
                        Review
                    @else
                        Verifikasi
                    @endif

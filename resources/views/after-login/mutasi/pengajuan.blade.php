...
@section('content')
                                ...
                                <td>
                                    <div class="aksi">
                                        @if (isAllAdmin())
                                            @if (isAVPAdm())
                                                <a href="##edit" id="edit"
                                                    onclick="modalEditMutasi(
                                                        '{{ route('mutasi.update', ['id' => $item->id]) }}',
                                            ...
                                            <a href="{{ route('mutasi.detail', ['id' => $item->id]) }}">
                                                <x-svg.icon.info />
                                            </a>
                                            {{-- @if (isAVPAdm())
                                                <x-layout.delete action="{{ route('mutasi.destroy', ['id' => $item->id]) }}" />
                                            @endif --}}
                                        @endif
                                    </div>
                                </td>
    ...
    @if (isAllAdmin())
        <!-- Modal -->
        <x-modals.admin action="{{route('mutasi.store')}}">
            @slot('slotHeader')
        ...
        <x-modals.admin action="{{route('mutasi')}}" id="editModal" isUpdate=true>
            @slot('slotHeader')

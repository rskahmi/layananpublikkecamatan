    ...
    <div class="row detail-pengajuan">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-6 mt-2 mt-sm-2 mt-md-3">
                            <x-text.PopUpMenu title="Tanggal Diajukan" subtitle="{{ format_dfy($mutasi->tanggal) }}" />
                        </div>
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-6 mt-2 mt-sm-2 mt-md-3 popup-text">
                            <h6>Memo TEO </h6>
                            <a target="_blank" href="{{ asset('storage/mutasi/' . $mutasi->memoteo) }}">
                        ...
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-6 mt-2 mt-sm-2 mt-md-3 popup-text">
                            ...
                            <span
                                ...
                                {{ isStatusDiterima($mutasi->status) ? 'bg-success' : (isStatusProses($mutasi->status) ? 'bg-warning' : (isStatusDitolak($mutasi->status) ? 'bg-danger' : 'bg-warning-subtle text-dark')) }}">{{ $mutasi->status }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div
            ...
            <div class="container ms-0 ms-sm-0 ms-md-0 ms-lg-0 ms-xl-5">
                ...
                @if (isset($riwayat_mutasi))
                    @if (count($riwayat_mutasi) > 0)
                        <div
                            class="row {{ (isStatusDiterima($mutasi->status) || isStatusDitolak($mutasi->status)) ? 'mt-5' : '' }}">
                            <div class="col-12">
        ...
        <x-modals.admin id="modalVerifikasiPengajuan" action="{{ route('mutasi.verifikasi', ['id' => $mutasi->id]) }}"
            class="modal-xl">
            @slot('slotHeader')
                <h5 class="modal-title" id="exampleModalLabel">
                    @if ($mutasi !== null && isStatusDiajukan($mutasi->status))
                        Review
                    @else
                        Verifikasi
                    @endif

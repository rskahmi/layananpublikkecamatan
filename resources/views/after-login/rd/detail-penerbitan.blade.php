    ...
    <div class="row detail-pengajuan">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-6 mt-2 mt-sm-2 mt-md-3">
                            <x-text.PopUpMenu title="Jenis" subtitle="{{ $penerbitan->jenis }}" />
                        </div>
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-6 mt-2 mt-sm-2 mt-md-3">
                            <x-text.PopUpMenu title="Tanggal Diajukan" subtitle="{{ format_dfy($penerbitan->tanggal) }}" />
                        </div>
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-6 mt-2 mt-sm-2 mt-md-3 popup-text">
                            <h6>Status</h6>
                            <span
                                class="badge text-capitalize
                                {{ isStatusDiterima($penerbitan->status) ? 'bg-success' : (isStatusProses($penerbitan->status) ? 'bg-warning' : (isStatusDitolak($penerbitan->status) ? 'bg-danger' : 'bg-warning-subtle text-dark')) }}">{{ $penerbitan->status }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div
            ...
            <div class="container ms-0 ms-sm-0 ms-md-0 ms-lg-0 ms-xl-5">
                ...
                @if (isset($riwayat_penerbitan))
                    @if (count($riwayat_penerbitan) > 0)
                        <div
                            class="row {{ (isStatusDiterima($penerbitan->status) || isStatusDitolak($penerbitan->status)) ? 'mt-5' : '' }}">
                            <div class="col-12">
                                <h6 class="riwayat-header">Riwayat</h6>
                            </div>
                            <div class="col-12 mt-3">
                                <ul class="timeline">
                                    @foreach ($riwayat_penerbitan as $item)
        ...
        <x-modals.admin id="modalVerifikasiPengajuan" action="{{ route('penerbitan.verifikasi', ['id' => $penerbitan->id]) }}"
            class="modal-xl">
            @slot('slotHeader')
                <h5 class="modal-title" id="exampleModalLabel">
                    @if ($penerbitan !== null && isStatusDiajukan($penerbitan->status))
                        Review
                    @else
                        Verifikasi
                    @endif

...
@section('content')
                    ...
                    @slot('slotBody')
                        ...
                        @foreach($rd as $item)
                            <tr>
                                ...
                                <td>
                                    @if ($item->baru !== null)
                                        <span class="badge text-capitalize {{ isStatusDiterima($item->baru->status) ? 'bg-success' : (isStatusProses($item->baru->status) ? 'bg-warning' : (isStatusDiajukan($item->baru->status) ? 'bg-warning-subtle text-dark' : 'bg-danger')) }}">{{ $item->baru->status }}</span>
                                    @elseif ($item->ganti !== null)
                                        <span class="badge text-capitalize {{ isStatusDiterima($item->ganti->status) ? 'bg-success' : (isStatusProses($item->ganti->status) ? 'bg-warning' : (isStatusDiajukan($item->ganti->status) ? 'bg-warning-subtle text-dark' : 'bg-danger')) }}">{{ $item->ganti->status }}</span>
                                    @elseif ($item->kembalikan !== null)
                                        <span class="badge text-capitalize {{ isStatusDiterima($item->kembalikan->status) ? 'bg-success' : (isStatusProses($item->kembalikan->status) ? 'bg-warning' : (isStatusDiajukan($item->kembalikan->status) ? 'bg-warning-subtle text-dark' : 'bg-danger')) }}">{{ $item->kembalikan->status }}</span>
                                    @else
                                        -
                                    @endif
                                </td>
                                <td>
                                    <div class="aksi">
                                        @if (isAllAdmin())
                                            ...
                                            <a href="{{route('rd.detail', ['id' => $item->id])}}">
                                                <x-svg.icon.info />
                                            </a>
                                            {{-- @if (isAdmin())
    ...
    @if (isAllAdmin())
        <!-- Modal -->
        <x-modals.admin action="{{route('rd.store')}}">
            @slot('slotHeader')
        ...
        <x-modals.admin action="{{route('rd')}}" id="editModal" isUpdate=true>
            @slot('slotHeader')

    ...
    <div class="row detail-pengajuan">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-6 mt-2 mt-sm-2 mt-md-3">
                            <x-text.PopUpMenu title="Tanggal Diajukan" subtitle="{{ format_dfy($rotasi->tanggal) }}" />
                        </div>
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-6 mt-2 mt-sm-2 mt-md-3 popup-text">
                            <h6>Memo TEO </h6>
                            <a target="_blank" href="{{ asset('storage/rotasi/' . $rotasi->memoteo) }}">
                        ...
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-6 mt-2 mt-sm-2 mt-md-3 popup-text">
                            ...
                            <span
                                ...
                                {{ isStatusDiterima($rotasi->status) ? 'bg-success' : (isStatusProses($rotasi->status) ? 'bg-warning' : (isStatusDitolak($rotasi->status) ? 'bg-danger' : 'bg-warning-subtle text-dark')) }}">{{ $rotasi->status }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div
            ...
            <div class="container ms-0 ms-sm-0 ms-md-0 ms-lg-0 ms-xl-5">
                ...
                @if (isset($riwayat_rotasi))
                    @if (count($riwayat_rotasi) > 0)
                        <div
                            class="row {{ (isStatusDiterima($rotasi->status) || isStatusDitolak($rotasi->status)) ? 'mt-5' : '' }}">
                            <div class="col-12">
                                <h6 class="riwayat-header">Riwayat</h6>
                            </div>
                            <div class="col-12 mt-3">
                                <ul class="timeline">
                                    @foreach ($riwayat_rotasi as $item)
        ...
        <x-modals.admin id="modalVerifikasiPengajuan" action="{{ route('rotasi.verifikasi', ['id' => $rotasi->id]) }}"
            class="modal-xl">
            @slot('slotHeader')
                <h5 class="modal-title" id="exampleModalLabel">
                    @if ($rotasi !== null && isStatusDiajukan($rotasi->status))
                        Review
                    @else
                        Verifikasi
                    @endif

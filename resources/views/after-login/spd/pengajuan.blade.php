...
@section('content')
                                ...
                                <td>
                                    <div class="aksi">
                                        @if (isAllAdmin())
                                            @if (isAdmin())
                                                <a href="##edit" id="edit"
                                                    onclick="modalEditSPD(
                                                        '{{ route('spd.update', ['id' => $item->id]) }}',
                                                        '{{ addslashes($item->tanggalberangkat) }}',
                                                        '{{ addslashes($item->tanggalpulang) }}',
                                            ...
                                            <a href="{{route('spd.detail', ['id' => $item->id])}}">
                                                <x-svg.icon.info />
                                            </a>
                                            {{-- @if (isAdmin())
    ...
    @if (isAllAdmin())
        <!-- Modal -->
        <x-modals.admin action="{{route('spd.store')}}">
            @slot('slotHeader')
                <h5 class="modal-title" id="exampleModalLabel">Tambah Pengajuan SPD</h5>
            @endslot
        ...
        <x-modals.admin action="{{route('spd')}}" id="editModal" isUpdate=true>
            @slot('slotHeader')

    ...
    <div class="row detail-pengajuan">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-6 mt-2 mt-sm-2 mt-md-3">
                            <x-text.PopUpMenu title="Tanggal Diajukan" subtitle="{{ format_dfy($spd->tanggal) }}" />
                        </div>
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-6 mt-2 mt-sm-2 mt-md-3">
                            <x-text.PopUpMenu title="Tanggal Berangkat" subtitle="{{ format_dfy($spd->tanggalberangkat) }}" />
                        </div>
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-6 mt-2 mt-sm-2 mt-md-3">
                            <x-text.PopUpMenu title="Tanggal Pulang" subtitle="{{ format_dfy($spd->tanggalpulang) }}" />
                        </div>
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-6 mt-2 mt-sm-2 mt-md-3">
                            <x-text.PopUpMenu title="Tujuan" subtitle="{{ $spd->tujuan }}" />
                        </div>
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-6 mt-2 mt-sm-2 mt-md-3 popup-text">
                            ...
                            <a target="_blank" href="{{ asset('storage/spd/' . $spd->lampiran) }}">
                        ...
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-6 mt-2 mt-sm-2 mt-md-3 popup-text">
                            ...
                            <span
                                ...
                                {{ isStatusDiterima($spd->status) ? 'bg-success' : (isStatusProses($spd->status) ? 'bg-warning' : (isStatusDitolak($spd->status) ? 'bg-danger' : 'bg-warning-subtle text-dark')) }}">{{ $spd->status }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div
            ...
            <div class="container ms-0 ms-sm-0 ms-md-0 ms-lg-0 ms-xl-5">
                ...
                @if (isset($riwayat_spd))
                    @if (count($riwayat_spd) > 0)
                        <div
                            class="row {{ (isStatusDiterima($spd->status) || isStatusDitolak($spd->status)) ? 'mt-5' : '' }}">
                            <div class="col-12">
                                <h6 class="riwayat-header">Riwayat</h6>
                            </div>
                            <div class="col-12 mt-3">
                                <ul class="timeline">
                                    @foreach ($riwayat_spd as $item)
        ...
        <x-modals.admin id="modalVerifikasiPengajuan" action="{{ route('spd.verifikasi', ['id' => $spd->id]) }}"
            class="modal-xl">
            @slot('slotHeader')
                <h5 class="modal-title" id="exampleModalLabel">
                    @if ($spd !== null && isStatusDiajukan($spd->status))
                        Review
                    @else
                        Verifikasi
                    @endif
